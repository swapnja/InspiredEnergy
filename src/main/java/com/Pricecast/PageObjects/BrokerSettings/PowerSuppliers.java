...
public class PowerSuppliers {

	WebDriverHandler _webDriverHandler;
	public PowerSuppliers (WebDriverHandler webDriverHandler) {
		_webDriverHandler = webDriverHandler;
		buttonSettings = _webDriverHandler.byXpath("//*[@id='btnSetting']");
		linkPowerSuppliers = _webDriverHandler.byXpath("//a[@data-action='PowerSupplier']");
		...
		btnCancel = _webDriverHandler.byXpath("//*[@class='btn btn-sm btn-danger']"); ////*[@id='editPowerSupplierForm']//button[@class='btn btn-sm btn-danger']
	...
	ElementHandler linkPowerSuppliers;
	...
	String[] arrTblLabelsPowerSuppliers = new String[8];
	String[] tblPowerSuppliers = new String[] { "Supplier", "Power Matrix Upload?", "Bespoke Power Pricing?", "Quote Text", "Min Uplift", "Max Uplift", "Uplift Increment", "Actions" };
	...
	public void selectSettings() {
		...
		waitLoad();
		linkPowerSuppliers.waitClickable().click();
	}

	public void columnVerification(){
		for(int i = 0 ; i < arrTblLabelsPowerSuppliers.length ; i++) {
			arrTblLabelsPowerSuppliers[i] = _webDriverHandler.byXpath("//table[@id='tblPowerSupplier']/thead/tr/th[" + (i+1) + "]").getText();
			if(!(tblPowerSuppliers[i].equals(arrTblLabelsPowerSuppliers[i])))
			{
	...
	public void toggleMatrixUpload(boolean Status){
		if(Status){
			if(!matrixUploadtglCheck.isSelected()){
				...
				System.out.println("\nPower Matrix Upload is now enabled.");
			}
			else {
				System.out.println("\nPower Matrix Upload is already enabled.");
			}
		}
		else {
			if(matrixUploadtglCheck.isSelected()){
				matrixUploadtgl.click();
				System.out.println("\nPower Matrix Upload is now disabled.");
			}
			else {
				System.out.println("\nPower Matrix Upload is already disabled.");
			}
		}
	}
