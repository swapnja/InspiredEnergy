...
public class SubCategories {
    ...
    public SubCategories(WebDriverHandler webDriverHandler) {
        ...
        btnCreate = _webDriverHandler.byXpath("//form[@id = 'modalAddNewSubCategory']//*[contains(text(), ' Create')]");
    ...
    public String[] tblSubCategories = new String[]{"Status", "Sub-Category Name", "Category Name", "Customer", "Supplier", "Licensee", "Global", "Update on Create", "Next Step", "Next Action Days", "Actions"};
    ...
    public void subCategoriesForm (Hashtable<String, String> formData) {
        tbSubCategoryName.waitClickable().clear().sendKeys(formData.get("name"));
        ...
        tbDueDate.clear().sendKeys(formData.get("dueDate"));
        ddSLA.selectByText(formData.get("actionSLA"));
        tbActionDate.clear().sendKeys(formData.get("actionDate"));
    }

    public void createSubCategories(Hashtable<String, Boolean> tglData,  Hashtable<String, String> formData, String... toClose) {
    ...
    public void editSubCategories(String name, Hashtable<String, Boolean> tglData, Hashtable<String, String> formData, String... toClose)  {
        search(name);
    ...
    public void copySubCategories(String name, Hashtable<String, Boolean> tglData, Hashtable<String, String> formData, String... toClose)  {
        search(name);
        btnCopy.waitClickable().click();
        Assert.assertEquals("Missing Data", ddCategory.getSelectedText());
        Assert.assertEquals("Project Manager", ddDefaultAssignment.getSelectedText());
        Assert.assertEquals("Resolve", ddNextStep.getSelectedText());
        Assert.assertEquals("78", tbDueDate.getAttribute("value"));
        Assert.assertEquals("Days from Action", ddSLA.getSelectedText());
        Assert.assertEquals("59", tbActionDate.getAttribute("value"));
        Assert.assertTrue(tglSupplierRequiredCheck.isSelected());
        Assert.assertFalse(tglUtilityRequiredCheck.isSelected());
        Assert.assertTrue(tglCustomerAccessCheck.isSelected());
        Assert.assertFalse(tglSupplierAccessCheck.isSelected());
        Assert.assertTrue(tglLicenseeAccessCheck.isSelected());
        Assert.assertFalse(tglGlobalCheck.isSelected());
        Assert.assertTrue(tglDueWorkDaysCheck.isSelected());
        Assert.assertFalse(tglActionWorkDaysCheck.isSelected());
        Assert.assertTrue(tglStatusCheck.isSelected());
        Assert.assertFalse(tglCopyFieldsCheck.isSelected());
        Assert.assertFalse(tglCopyActionsCheck.isSelected());
        subCategoriesTgl(tglData);
        subCategoriesForm(formData);
        ddToClose.waitClickable().click();
        for(int i=1; i<5; i++) {
...
//        search(formData.get("name"));
    ...
    public void errorValidation() {
        btnNewSubCategory.waitClickable().click();
        btnCreate.waitClickable().click();
        if (msgSubName.getText().equalsIgnoreCase(strErrorSubName) && msgCategory.getText().equals(strErrorCategory) && msgNxtStep.getText().equals(strErrorNxtStep) && msgDueDate.getText().equals(strErrorDueDate)
        && msgSLA.getText().equals(strErrorSLA) && msgActionDate.getText().equals(strErrorActionDate) && msgToClose.getText().equals(strErrorToClose) && msgDefaultAssignment.getText().equals(strErrorDefaultAssignment)) {
