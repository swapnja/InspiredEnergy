...
public class Accounts {
    ...
    public Accounts (WebDriverHandler webDriverHandler) {
        ...
        tabAccounts = _webDriverHandler.byXpath("//a[@href = '#AccountsDiv']");

        btnAccounts = _webDriverHandler.byXpath("//*[@onclick = 'showAccountsForm()']");
        ...
        btnApplyFilters = _webDriverHandler.byXpath("//button[@onclick = 'bindAccountsLinksTable()']");
        tbSearch = _webDriverHandler.byXpath("//input[@type = 'search']");
        btnConfirm = _webDriverHandler.byXpath("//form[@id = 'queryTaskAccountsForm']//button[contains(text(), 'Confirm')]");
        btnCancel = _webDriverHandler.byXpath("//form[@id = 'queryTaskAccountsForm']//button[contains(text(), 'Cancel')]");
        btnYes = _webDriverHandler.byXpath("//button[contains(text(), 'Yes')]");
        btnOK = _webDriverHandler.byXpath("//button[contains(text(), 'OK')]");
    }

    public String[] tblAccounts = new String[]{"Added date", "Added By", "Account Number", "Utility", "Supplier", "Link Status", "Actions"};
    public String[] arrAccounts = new String[7];
    ...
    public void accessAddAccounts() {
        btnAccounts.waitClickable().click();
        waitLoad();
    }

    public void columnVerification() {
        for (int i = 0; i < arrAccounts.length; i++) {
            arrAccounts[i] = _webDriverHandler.byXpath("//table[@id='TaskAccountsTable']/thead/tr/th[" + (i + 1) + "]").getText();
            if (!(tblAccounts[i].equals(arrAccounts[i]))) {
                System.out.println(tblAccounts[i]);
                System.out.println(arrAccounts[i]);
    ...
    public void accountFilters(String type, String... data) {
        switch (type) {
            case "Supplier":
    ...
    public void accountFiltersReset() {
    ...
    public void accountSearch(String accountID) {
        tbSearch.waitClickable().clear().sendKeys(accountID);
        waitLoad();
    }

    public void addAccount(String accountID) {
        accountSearch(accountID);
        _webDriverHandler.byXpath("//tr[td[contains(text(), '"+ accountID +"')]]//input").waitClickable().click();
        btnConfirm.waitClickable().click();
        btnOK.waitClickable().click();
        waitLoad();
    }

    public void verifySelection(String accountID) {
        btnAccounts.waitClickable().click();
        addAccount(accountID);
        btnAccounts.waitClickable().click();
        accountSearch(accountID);
        Assert.assertTrue(_webDriverHandler.byXpath("//tr[td[contains(text(), '"+ accountID +"')]]//input").notPresent(), "Already selected entries are not shown.");
    }

    public void actionAccount(String actionType, String accountID) {
        if (actionType.equalsIgnoreCase("Delete")) {
            _webDriverHandler.byXpath("//tr[td[contains(text(), '" + accountID + "')]]//button[@title = '']").waitClickable().click();
        } else if (actionType.equalsIgnoreCase("Completed")) {
            _webDriverHandler.byXpath("//tr[td[contains(text(), '" + accountID + "')]]//button[@title = 'Mark Completed']").waitClickable().click();
        }
        else{
    ...
    public void errorValidation() {
        ...
        Assert.assertEquals("Please select accounts to link.", errorMsg);
        btnOK.waitClickable().click();
